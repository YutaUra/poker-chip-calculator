...
describe("PokerChipCalculator", () => {
  ...
  describe("エフェクティブスタック", () => {
    it("初期状態で Effective Stack トグルボタンが表示される", () => {
      render(<PokerChipCalculator />)
      expect(screen.getByText("Effective Stack")).toBeInTheDocument()
    })

    it("初期状態ではエフェクティブスタックの詳細が非表示である", () => {
      render(<PokerChipCalculator />)
      expect(screen.queryByText(/Opponent Stack/)).not.toBeInTheDocument()
      expect(screen.queryByText(/Effective Stack:/)).not.toBeInTheDocument()
    })

    it("トグルボタンをクリックすると詳細が展開される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      expect(screen.getByText("Opponent Stack (BB):")).toBeInTheDocument()
      expect(screen.getByText(/Effective Stack: .+ BB/)).toBeInTheDocument()
    })

    it("展開時にエフェクティブスタックが正しく表示される（自分10BB、相手100BB）", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      expect(screen.getByText("Effective Stack: 10 BB")).toBeInTheDocument()
    })

    it("相手スタックBBを変更するとエフェクティブスタックが再計算される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      const opponentInput = screen.getByLabelText("Opponent Stack (BB):")
      fireEvent.change(opponentInput, { target: { value: "5" } })
      expect(screen.getByText("Effective Stack: 5 BB")).toBeInTheDocument()
    })

    it("トグル状態が sessionStorage に永続化される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      expect(JSON.parse(sessionStorage.getItem("effective-stack-enabled")!)).toBe(true)
    })

    it("相手スタック値が sessionStorage に永続化される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      const opponentInput = screen.getByLabelText("Opponent Stack (BB):")
      fireEvent.change(opponentInput, { target: { value: "50" } })
      expect(JSON.parse(sessionStorage.getItem("opponent-stack-bb")!)).toBe(50)
    })

    it("エフェクティブスタックが整数の場合は小数なしで表示される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      expect(screen.getByText("Effective Stack: 10 BB")).toBeInTheDocument()
    })

    it("エフェクティブスタックが小数の場合は小数1桁で表示される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Effective Stack"))
      const opponentInput = screen.getByLabelText("Opponent Stack (BB):")
      fireEvent.change(opponentInput, { target: { value: "7.5" } })
      expect(screen.getByText("Effective Stack: 7.5 BB")).toBeInTheDocument()
    })
    ...
    it("再度トグルすると詳細が閉じる", () => {
      ...
      expect(screen.getByText("Opponent Stack (BB):")).toBeInTheDocument()
      fireEvent.click(screen.getByText("Effective Stack"))
      expect(screen.queryByText("Opponent Stack (BB):")).not.toBeInTheDocument()
    })
  })

  describe("Pot Odds セクション", () => {
    it("初期状態で Pot Odds ボタンが表示される", () => {
      render(<PokerChipCalculator />)
      expect(screen.getByText("Pot Odds")).toBeInTheDocument()
    })

    it("初期状態では入力フィールドが非表示（折りたたみ）", () => {
      render(<PokerChipCalculator />)
      expect(screen.queryByLabelText("Pot Size (BB):")).not.toBeInTheDocument()
      expect(screen.queryByLabelText("Call Amount (BB):")).not.toBeInTheDocument()
    })

    it("トグルボタンをクリックすると入力フィールドが表示される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Pot Odds"))
      expect(screen.getByLabelText("Pot Size (BB):")).toBeInTheDocument()
      expect(screen.getByLabelText("Call Amount (BB):")).toBeInTheDocument()
    })

    it("トグルボタンに aria-expanded 属性が設定される", () => {
      render(<PokerChipCalculator />)
      const toggleButton = screen.getByText("Pot Odds").closest("button")!
      expect(toggleButton).toHaveAttribute("aria-expanded", "false")
      fireEvent.click(toggleButton)
      expect(toggleButton).toHaveAttribute("aria-expanded", "true")
    })

    it("入力値に応じてポットオッズが表示される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Pot Odds"))
      const potSizeInput = screen.getByLabelText("Pot Size (BB):")
      const callAmountInput = screen.getByLabelText("Call Amount (BB):")
      fireEvent.change(potSizeInput, { target: { value: "6" } })
      fireEvent.change(callAmountInput, { target: { value: "2" } })
      expect(screen.getByText("Pot Odds: 25%")).toBeInTheDocument()
      expect(screen.getByText("Required Equity: 25%")).toBeInTheDocument()
    })

    it("端数ありの場合は小数1桁で表示される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Pot Odds"))
      const potSizeInput = screen.getByLabelText("Pot Size (BB):")
      const callAmountInput = screen.getByLabelText("Call Amount (BB):")
      fireEvent.change(potSizeInput, { target: { value: "10" } })
      fireEvent.change(callAmountInput, { target: { value: "5" } })
      expect(screen.getByText("Pot Odds: 33.3%")).toBeInTheDocument()
      expect(screen.getByText("Required Equity: 33.3%")).toBeInTheDocument()
    })
    ...
    it("入力値が sessionStorage に永続化される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Pot Odds"))
      const potSizeInput = screen.getByLabelText("Pot Size (BB):")
      const callAmountInput = screen.getByLabelText("Call Amount (BB):")
      fireEvent.change(potSizeInput, { target: { value: "10" } })
      fireEvent.change(callAmountInput, { target: { value: "5" } })
      expect(JSON.parse(sessionStorage.getItem("pot-size-bb")!)).toBe(10)
      expect(JSON.parse(sessionStorage.getItem("call-amount-bb")!)).toBe(5)
    })

    it("sessionStorage から状態を復元する", () => {
      sessionStorage.setItem("pot-odds-enabled", JSON.stringify(true))
      sessionStorage.setItem("pot-size-bb", JSON.stringify(6))
      sessionStorage.setItem("call-amount-bb", JSON.stringify(2))
      render(<PokerChipCalculator />)
      expect(screen.getByLabelText("Pot Size (BB):")).toBeInTheDocument()
      expect(screen.getByText("Pot Odds: 25%")).toBeInTheDocument()
      expect(screen.getByText("Required Equity: 25%")).toBeInTheDocument()
    ...
    it("初期状態（コール額0）では Pot Odds: 0% と表示される", () => {
      render(<PokerChipCalculator />)
      fireEvent.click(screen.getByText("Pot Odds"))
      expect(screen.getByText("Pot Odds: 0%")).toBeInTheDocument()
      expect(screen.getByText("Required Equity: 0%")).toBeInTheDocument()
    })
  })
